...
class RTC_EXPORT PeerConnectionInterface : public rtc::RefCountInterface {
 public:
  ...
  struct RTC_EXPORT RTCConfiguration {
    ...
    static const int kAudioJitterBufferMaxPackets = 200;                       //^Here defines the Jitter buffer size.
    ...
    int audio_jitter_buffer_max_packets = kAudioJitterBufferMaxPackets;        //^Alocate default Jitter buffer size to the variable

    // !Whether to use the NetEq "fast mode" which will accelerate audio quicker
    // !if it falls behind.
    bool audio_jitter_buffer_fast_accelerate = false;                          //^?What will be the prepose?
