...
// !Jitter delay statistical algorithm
...
// !Jitter delay statistical algorithm
void BufferLevelFilter::Update(size_t buffer_size_samples,      // ^Current buffer size
                               int time_stretched_samples) {    // ^for calculating the inflence of speeding up or slowing down
  // Filter:
  ...
  const int64_t filtered_current_level =                                        
      (level_factor_ * int64_t{filtered_current_level_} >> 8) +             // ^Calculate smoothing delay using a dynamic forgetting factor (calculated from the network delay value)
  ...
  filtered_current_level_ = rtc::saturated_cast<int>(std::max<int64_t>(     // ^calculate the inflence of speeding up or slowing down
      0, filtered_current_level - int64_t{time_stretched_samples} * (1 << 8)));
}

// !Adapt buffer level according to the target buffer level
