...
void PacketBuffer::PartialFlush(int target_level_ms,
  ...
  while (GetSpanSamples(last_decoded_length, sample_rate, true) >
         ...
         buffer_.size() > max_number_of_packets_ / 2) {                 //^We should avoid flushing to very low levels
...
size_t PacketBuffer::GetSpanSamples(size_t last_decoded_length,                //^talked: about the samples time length between back and front of buffer
                                    size_t sample_rate,
