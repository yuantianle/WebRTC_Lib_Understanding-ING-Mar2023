...
NetEqImpl::Dependencies::Dependencies(                                         //^function for setting parameters
...
int NetEqImpl::InsertPacket(const RTPHeader& rtp_header,
  ...
  TRACE_EVENT0("webrtc", "NetEqImpl::InsertPacket");                           //^trace event with its catgory and name 
  MutexLock lock(&mutex_);
  if (InsertPacketInternal(rtp_header, payload) != 0) {                        //^call corresponding private function
    return kFail;                                                              //^enum: -1
  } 
  return kOK;                                                                  //^enum: 0
}
...
int NetEqImpl::InsertPacketInternal(const RTPHeader& rtp_header,
  ...
  if (payload.empty()) {                                                      //^Report invalid if payload is empty 
    RTC_LOG_F(LS_ERROR) << "payload is empty";
    return kInvalidPointer;
  }
  
  Timestamp receive_time = clock_->CurrentTime();                             //^Record current time (Timestamp represents the time that has
  stats_->ReceivedPacket();                                                   //^passed since some unspecified epoch. from timestamp.h)

  //!1. Put the data into the local variable PacketList
  PacketList packet_list;
  // Insert packet in a packet list.
  
  packet_list.push_back([&rtp_header, &payload, &receive_time] {              //^Put the RTP data into the local variable PacketList
    // Convert to Packet.
    Packet packet;
  ...
  //!2. Process the RTP package logic:
  //!2.a Convert internal and external timestamps 
  bool update_sample_rate_and_channels = first_packet_;                        //^Set to wait for new codec (first time packet transmit)

  if (update_sample_rate_and_channels) {                                       //^If true, reset timestamp scaling
    // Reset timestamp scaling.
    timestamp_scaler_->Reset();
  }

  if (!decoder_database_->IsRed(rtp_header.payloadType)) {                     //^`rtp_payload_type` is NOT registered as RED.
    // Scale timestamp to internal domain (only for some codecs).
    timestamp_scaler_->ToInternal(&packet_list);                               //^Scale timestamps for codecs that need timestamp scaling.
  }

  // Store these for later use, since the first packet may very well disappear    
  // before we need these values.
  uint32_t main_timestamp = packet_list.front().timestamp;                     //^Back up the information of the first package
  ...
  if (update_sample_rate_and_channels) {                                       //^Initialize when changed SSRC or first call
    // Note: `first_packet_` will be cleared further down in this method, once
    // the packet has been successfully inserted into the packet buffer.

    // Flush the packet buffer and DTMF buffer.
    packet_buffer_->Flush(stats_.get());                                       //^Use the flush function in packet_buffer.h to clean packet buffer
    dtmf_buffer_->Flush();                                                     //^Flush DTMF event data

    // Update audio buffer timestamp.
    sync_buffer_->IncreaseEndTimestamp(main_timestamp - timestamp_);

    // Update codecs.
    timestamp_ = main_timestamp;                                               //^Reinitialize timestamp using main_timestamp
  }

  //!2.b NACK (Negative Acknowledgement) processing
  if (nack_enabled_) {                                                         //^NACK check. If nack is enabled, nack statistics are collected. 
    RTC_DCHECK(nack_);                                                         
    if (update_sample_rate_and_channels) {
      nack_->Reset();                                                          //^If packet loss occurs, nack retransmission is sent
    }
    nack_->UpdateLastReceivedPacket(main_sequence_number, main_timestamp);
  }

  //!2.c Redundant large packet determination (RED) and decompose to small packet
  // Check for RED payload type, and separate payloads into several packets.      
  if (decoder_database_->IsRed(rtp_header.payloadType)) {                      //^Classify payloads according to size
  ...
  // after RED splitting.
  if (decoder_database_->IsRed(rtp_header.payloadType)) {                      //^Back-up refreshed package data
  ...
  //!2.e Judge and process DTMF(dual - tone multi - frequency) packets
  // Process DTMF payloads. Cycle through the list of packets, and pick out any
  // DTMF payloads found.
  PacketList::iterator it = packet_list.begin();                               //^Cycle through the list of packets
  while (it != packet_list.end()) {
    const Packet& current_packet = (*it);
    RTC_DCHECK(!current_packet.payload.empty());
    if (decoder_database_->IsDtmf(current_packet.payload_type)) {              //^Find DTMF packet
      DtmfEvent event;
      int ret = DtmfBuffer::ParseEvent(current_packet.timestamp,               //^Prase DTMF event
  ...
  //!3. Packet analysis:
  PacketList parsed_packet_list;   //^There may be multiple decoded data in one packet,
  bool is_dtx = false;             //^which is separated here and saved in parsed_packet_list
  while (!packet_list.empty()) {   //^Transfer data: packet_list -> parsed_packet_list
    ...
    //!3.a Remove noisy packets
    if (info->IsComfortNoise()) {                                              //^Noise or not, remove noise
      // Carry comfort noise packets along.
      parsed_packet_list.splice(parsed_packet_list.end(), packet_list,
                                packet_list.begin());
    } else { //!3.b Obtain packet information by decode packet header
      ...
      std::vector<AudioDecoder::ParseResult> results =                         //^noise removed data
      ...
      if (results.empty()) {
        ...
        for (auto& result : results) {
          ...
          if (first) {
            ...
            packet_list.front() = packet_from_result(result);                  //^move packet_list data into parsed_packet_list
            parsed_packet_list.splice(parsed_packet_list.end(), packet_list,
                                      packet_list.begin());
            first = false;
          } else {
  ...
  //!3.c Calculate the number of normal voice packets except error-correcting packets and redundant packets
  // Calculate the number of primary (non-FEC/RED) packets.
  const size_t number_of_primary_packets = std::count_if(
      parsed_packet_list.begin(), parsed_packet_list.end(),                    //^Iterate packets in  parsed_packet_list
      [](const Packet& in) { return in.priority.codec_level == 0; });          //^Calculate the number of non-FEC/RED packets.
  if (number_of_primary_packets < parsed_packet_list.size()) {                 //^if there is any FEC/RED packet
    stats_->SecondaryPacketsReceived(parsed_packet_list.size() -
                                     number_of_primary_packets);
  }

  //!4. Insert speech packets into the packet buffer (PacketBuffer)
  // Insert packets in buffer.
  const int target_level_ms = controller_->TargetLevelMs();          
  //^<<<<<<<<<<<<<<<<<<<<<<<<   IMPORTANT   <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
  const int ret = packet_buffer_->InsertPacketList(                            //^<<<<<<<<!!!Insert packet from parsed_packet_list into packet_buffer_
  ...
  if (ret == PacketBuffer::kFlushed) {
    ...
    new_codec_ = true;
    update_sample_rate_and_channels = true;                                    //^Fullflush: Reset DSP timestamp etc. if packet buffer flushed.
    buffer_flush_occured = true;
  } else if (ret == PacketBuffer::kPartialFlush) {
    // Forward sync buffer timestamp
    timestamp_ = packet_buffer_->PeekNextPacket()->timestamp;                  //^Partical fullflush: inherent timestamp.
  ...
  if (first_packet_) {                                                         //^Welcome new codec
    first_packet_ = false;
    // Update the codec on the next GetAudio call.
    new_codec_ = true;
  }

  if (current_rtp_payload_type_) {                                             //^Record current rtp payload type into log
  ...
  if (update_sample_rate_and_channels && !packet_buffer_->Empty()) {           //^
    ...
    const Packet* next_packet = packet_buffer_->PeekNextPacket();              //^Go to next packet
    RTC_DCHECK(next_packet);
    const int payload_type = next_packet->payload_type;                        //^Get the next RTP header from `packet_buffer_` to obtain the payload type
    size_t channels = 1;
    ...
    const DecoderDatabase::DecoderInfo* decoder_info =                         //^Returns a pointer to the DecoderInfo struct for `rtp_payload_type`.
        decoder_database_->GetDecoderInfo(payload_type);
    RTC_DCHECK(decoder_info);                                                  //^Record decoder info for payload_type
    ...
    if (nack_enabled_) {                                                       //^Record NACK info
      RTC_DCHECK(nack_);
  ...
  const DecoderDatabase::DecoderInfo* dec_info = 
      decoder_database_->GetDecoderInfo(main_payload_type);                    //^Record decoder info for main payload_type
  RTC_DCHECK(dec_info);  // Already checked that the payload type is known.

  NetEqController::PacketArrivedInfo info;
  info.is_cng_or_dtmf = dec_info->IsComfortNoise() || dec_info->IsDtmf();      //^Record packet arriving Info
  ...
  const bool should_update_stats = !new_codec_;                                //^Update statistics
  auto relative_delay =
